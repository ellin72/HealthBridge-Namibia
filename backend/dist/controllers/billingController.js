...
exports.getBillingStats = exports.updateInvoice = exports.createInvoice = exports.getInvoices = exports.generateInvoiceAfterConsultation = void 0;
const prisma_1 = require("../utils/prisma");
// Automatically generate invoice after consultation
const generateInvoiceAfterConsultation = async (appointmentId, providerId, patientId) => {
    try {
        // Check if invoice already exists for this appointment
        const existingInvoice = await prisma_1.prisma.billingInvoice.findFirst({
            where: { appointmentId }
        });
        if (existingInvoice) {
        ...
        const invoice = await prisma_1.prisma.billingInvoice.create({
            data: {
                providerId,
                patientId,
                appointmentId,
...
exports.generateInvoiceAfterConsultation = generateInvoiceAfterConsultation;
// Get invoices
const getInvoices = async (req, res) => {
    try {
        ...
        const invoices = await prisma_1.prisma.billingInvoice.findMany({
            where,
            include: {
                patient: {
                    select: { id: true, firstName: true, lastName: true, email: true },
...
const updateInvoice = async (req, res) => {
    try {
        ...
        const invoice = await prisma_1.prisma.billingInvoice.findFirst({
        ...
        const updated = await prisma_1.prisma.billingInvoice.update({
            where: { id },
            data: {
...
const getBillingStats = async (req, res) => {
    try {
        const userId = req.user.id;
        const userRole = req.user.role;
        const where = userRole === 'HEALTHCARE_PROVIDER'
            ? { providerId: userId }
            : { patientId: userId };
        const [totalInvoices, paidInvoices, pendingInvoices, overdueInvoices, totalRevenue] = await Promise.all([
            prisma_1.prisma.billingInvoice.count({ where }),
            prisma_1.prisma.billingInvoice.count({ where: { ...where, status: 'PAID' } }),
            prisma_1.prisma.billingInvoice.count({ where: { ...where, status: 'PENDING' } }),
            prisma_1.prisma.billingInvoice.count({
                where: {
                    ...where,
                    status: 'PENDING',
                    dueDate: { lt: new Date() },
            ...
            prisma_1.prisma.billingInvoice.aggregate({
