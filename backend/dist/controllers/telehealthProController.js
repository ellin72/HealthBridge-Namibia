...
exports.getProviderAnalytics = exports.updatePatientHistory = exports.getPatientHistory = exports.createPatientHistory = exports.updateVideoConsultation = exports.getVideoConsultation = exports.createVideoConsultation = void 0;
...
const createVideoConsultation = async (req, res) => {
    try {
        ...
        const videoConsultation = await prisma_1.prisma.videoConsultation.create({
            data: {
                appointmentId,
...
const getVideoConsultation = async (req, res) => {
    try {
        ...
        const videoConsultation = await prisma_1.prisma.videoConsultation.findUnique({
            where: { appointmentId },
            include: {
                appointment: {
                    include: {
...
exports.getVideoConsultation = getVideoConsultation;
const updateVideoConsultation = async (req, res) => {
    try {
        ...
        const updated = await prisma_1.prisma.videoConsultation.update({
            where: { appointmentId },
            data: updateData
        });
...
exports.updateVideoConsultation = updateVideoConsultation;
// ========== PATIENT HISTORY ==========
const createPatientHistory = async (req, res) => {
    try {
        ...
        const history = await prisma_1.prisma.patientHistory.create({
            data: {
                patientId,
...
const getPatientHistory = async (req, res) => {
    try {
        ...
        const histories = await prisma_1.prisma.patientHistory.findMany({
            where: { patientId },
...
const updatePatientHistory = async (req, res) => {
    try {
        ...
        const userId = req.user.id;
        const userRole = req.user.role;
        const history = await prisma_1.prisma.patientHistory.findUnique({
            where: { id }
        });
        if (!history) {
        ...
        if (medicalHistory !== undefined)
            updateData.medicalHistory = medicalHistory ? JSON.stringify(medicalHistory) : null;
        if (allergies !== undefined)
            updateData.allergies = allergies ? JSON.stringify(allergies) : null;
        if (medications !== undefined)
            updateData.medications = medications ? JSON.stringify(medications) : null;
        if (vitalSigns !== undefined)
            updateData.vitalSigns = vitalSigns ? JSON.stringify(vitalSigns) : null;
        if (labResults !== undefined)
            updateData.labResults = labResults ? JSON.stringify(labResults) : null;
        if (notes !== undefined)
            updateData.notes = notes;
        const updated = await prisma_1.prisma.patientHistory.update({
            where: { id },
            data: updateData
        });
...
const getProviderAnalytics = async (req, res) => {
    try {
        ...
        const totalAppointments = await prisma_1.prisma.appointment.count({ where });
        const confirmedAppointments = await prisma_1.prisma.appointment.count({
        ...
        const videoConsultations = await prisma_1.prisma.videoConsultation.count({
            where: {
                appointment: where
            }
        });
        // Get consultation notes count
        const consultationNotes = await prisma_1.prisma.consultationNote.count({
            where: {
                providerId: userRole === 'HEALTHCARE_PROVIDER' ? userId : undefined,
                appointment: where
            }
        ...
        const uniquePatients = await prisma_1.prisma.appointment.findMany({
            where,
...
//# sourceMappingURL=telehealthProController.js.map