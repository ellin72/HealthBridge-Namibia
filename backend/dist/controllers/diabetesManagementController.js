...
exports.getGlucoseStatistics = exports.addMedicationLog = exports.getGlucoseReadings = exports.addGlucoseReading = exports.updateDiabetesProgram = exports.getDiabetesProgramById = exports.getDiabetesPrograms = exports.createDiabetesProgram = void 0;
const prisma_1 = require("../utils/prisma");
// Create diabetes management program
const createDiabetesProgram = async (req, res) => {
    try {
        const userId = req.user.id;
        const { providerId, diabetesType, diagnosisDate, targetGlucoseRange, medications, insulinRegimen, mealPlan, exercisePlan, goals, } = req.body;
        if (!diabetesType) {
            return res.status(400).json({ message: 'Diabetes type is required' });
        }
        const program = await prisma_1.prisma.diabetesManagementProgram.create({
            data: {
                patientId: userId,
                providerId,
...
exports.createDiabetesProgram = createDiabetesProgram;
// Get patient's diabetes programs
const getDiabetesPrograms = async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const programs = await prisma_1.prisma.diabetesManagementProgram.findMany({
            where,
            include: {
                _count: {
                    select: {
...
exports.getDiabetesPrograms = getDiabetesPrograms;
// Get diabetes program by ID
const getDiabetesProgramById = async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const program = await prisma_1.prisma.diabetesManagementProgram.findFirst({
            where: {
                id,
                patientId: userId,
            },
...
exports.getDiabetesProgramById = getDiabetesProgramById;
// Update diabetes program
const updateDiabetesProgram = async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const { targetGlucoseRange, medications, insulinRegimen, mealPlan, exercisePlan, goals, status, } = req.body;
        const program = await prisma_1.prisma.diabetesManagementProgram.findFirst({
            where: {
                id,
                patientId: userId,
        ...
        const updated = await prisma_1.prisma.diabetesManagementProgram.update({
            where: { id },
            data: {
...
exports.updateDiabetesProgram = updateDiabetesProgram;
// Add glucose reading
const addGlucoseReading = async (req, res) => {
    try {
        const userId = req.user.id;
        const { programId, glucoseLevel, readingType, unit = 'mg/dL', mealContext, medicationTaken, notes, } = req.body;
        ...
        const reading = await prisma_1.prisma.glucoseReading.create({
            data: {
                programId,
                glucoseLevel,
                readingType,
...
const getGlucoseReadings = async (req, res) => {
    try {
        ...
        const readings = await prisma_1.prisma.glucoseReading.findMany({
            where,
            orderBy: {
                recordedAt: 'desc',
            },
...
const addMedicationLog = async (req, res) => {
    try {
        ...
        const log = await prisma_1.prisma.diabetesMedicationLog.create({
            data: {
                programId,
                medicationName,
                dosage,
...
const getGlucoseStatistics = async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const stats = {
            ...
            inRangeCount: targetRange
                ? readings.filter((r) => r.glucoseLevel >= targetRange.min &&
                    r.glucoseLevel <= targetRange.max).length
                : null,
...
exports.getGlucoseStatistics = getGlucoseStatistics;
//# sourceMappingURL=diabetesManagementController.js.map