...
exports.patientToFHIR = patientToFHIR;
exports.appointmentToFHIR = appointmentToFHIR;
exports.observationToFHIR = observationToFHIR;
exports.storeFHIRResource = storeFHIRResource;
exports.getFHIRResource = getFHIRResource;
exports.exportPatientAsFHIRBundle = exportPatientAsFHIRBundle;
...
function appointmentToFHIR(appointment) {
    return {
...
function observationToFHIR(observation) {
    return {
...
async function storeFHIRResource(resource) {
    const existing = await prisma_1.prisma.fHIRResource.findUnique({
        where: { fhirId: resource.id },
    });
    if (existing) {
        return await prisma_1.prisma.fHIRResource.update({
        ...
        return await prisma_1.prisma.fHIRResource.create({
            data: {
...
// Get FHIR resource
async function getFHIRResource(fhirId) {
    const resource = await prisma_1.prisma.fHIRResource.findUnique({
        where: { fhirId },
    });
    if (!resource) {
        return null;
...
async function exportPatientAsFHIRBundle(patientId) {
    const patient = await prisma_1.prisma.user.findUnique({
        where: { id: patientId },
        include: {
