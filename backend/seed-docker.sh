#!/bin/bash
# Run database seed from inside Docker to avoid Windows networking issues

echo "🌱 Running database seed from Docker container..."
...
if [ $? -eq 0 ]; then
  ...
  echo ""
  echo "❌ Seed failed. Please check the error messages above."
  exit 1
fi
